...
module.exports = class PestCommand {
    constructor(options) {
    ...
    get configuration() {
        let configFilepath = vscode.workspace.getConfiguration('better-pest').get('xmlConfigFilepath');
    ...
    get suffix() {
        let suffix = vscode.workspace.getConfiguration('better-pest').get('commandSuffix');
    ...
    get method() {
        ...
        while (line > 0) {
            ...
            const match = lineText.match(/^\s*it\(([^,)]+)/m);
            if (match) {
                method = match[1];
                break;
            }
